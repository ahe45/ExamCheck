...
`start-server.bat`은 웹 서버 실행 전에 Windows 방화벽 규칙 `ExamCheck-Web-TCP-5173`을 확인합니다. 규칙이 없거나 설정이 다르면 관리자 권한 요청 후 추가·수정하며, 올바른 규칙이 있으면 중복 생성하거나 권한을 다시 요청하지 않습니다. 모든 네트워크 프로필에서 같은 서브넷(`LocalSubnet`)의 TCP 5173 접속을 허용합니다. 관리자 권한은 방화벽 설정에만 사용하고 웹/API 서버는 원래 권한으로 실행합니다. 권한 요청을 취소하거나 설정에 실패하면 원인을 표시하고 실행을 중단합니다. 권한을 올린 설정 과정의 상세 로그는 `log/firewall.log`에 남습니다.

서버 PC에서는 열리지만 다른 PC에서 접속되지 않으면 PC 간 네트워크 연결과 조직의 방화벽 정책을 확인합니다. 다른 서브넷에서 접속해야 하는 환경은 관리자가 허용할 주소 범위를 별도로 정해야 합니다. 기본 구성에서는 브라우저가 API 포트 3100에 직접 접속하지 않습니다.
...
기본 프린터 모드는 실제 장치를 검색하는 `browser-print`입니다. Windows에서 Browser Print 프로그램을 실행하면 앱이 로컬 서비스에 직접 연결하여 프린터를 검색·선택·저장하고 테스트 라벨을 출력합니다. 별도의 `BrowserPrint.js` 배치는 필요하지 않으며, 자동화 테스트에서만 `VITE_PRINTER_MODE=mock`을 사용합니다. 프린터 전송은 성공 콜백 이후에도 물리 인쇄 완료가 아닌 “데이터 전송됨”으로 표시합니다.

출력 작업에 기록할 워크스테이션은 프린터 설정 화면에서 서버 등록 목록 중 하나를 선택해 PC별로 저장합니다.
`VITE_WORKSTATION_CODE`는 아직 저장된 선택값이 없는 PC에서 사용할 초기값이며, 로컬 기본값
`WS-DEV-001`은 초기 스키마에 등록됩니다.
