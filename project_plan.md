...
- 저장할 때 기존 양식을 덮어쓰지 않고 새 버전을 생성하며, 활성 버전만 사용자 화면에 제공한다.
...
현재 1차 구현 범위는 양식 버전 저장 API, 관리자 양식 편집기, 54개 데이터 태그, 기본 양식 2종, 가번호 부여 완료 후 사용자 양식 선택·출력이다. 고사실별·시험 전체 명단을 실제 데이터로 생성하는 화면과 집계 API는 후속 업무 모듈에서 연결한다.
