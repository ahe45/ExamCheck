...
describe("PseudonymRosterExporter", () => {
  it("creates the established roster sheet, columns, values, and styles", () => {
    ...
    const workbook = new PseudonymRosterExporter().createWorkbook([row], createdAt);
    ...
    expect(worksheet?.columns.map((column) => column.width)).toEqual([9, 14, 18, 16, 28, 28, 24, 12]);
    ...
    expect(worksheet?.autoFilter).toEqual({ from: "A1", to: "H1" });
  });

  it("serializes a valid XLSX buffer with the same visible sheet data", async () => {
    const buffer = await new PseudonymRosterExporter().build([row]);
    ...
    expect(worksheet?.getCell("H2").value).toBe("등록");
  });
});
