...
`050_default_operation_form_templates.sql`은 2026-09-16 양식 관리에 저장된 **가번호 부여대장**, **결시자 명단**, **수험생 사진대장**을 기본 항목으로 추가한다. 문서 내용, 데이터태그의 표시 형식, 데이터 블록, 여백, 페이지 번호 등 저장된 편집 설정을 함께 포함한다.

`051_remove_legacy_default_form_templates.sql`은 기존 기본 양식 코드 `PSEUDONYM_SLIP`(가번호표), `CANDIDATE_CONFIRMATION`(수험생 확인표)을 삭제한다. 새 설치의 기본 양식은 위 세 개만 남는다. 기존 서버에서도 같은 두 코드만 삭제하며, 다른 코드로 직접 만든 양식과 나머지 양식의 수정 내용은 유지한다.
